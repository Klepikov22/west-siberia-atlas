...
for ix,seg in enumerate(score_segments[:300]):
    if ix and ix%100==0: print('match',ix,'/',len(score_segments),'cand',len(candidates),flush=True)
    tol=seg['tol']*1000; radius=tol+TARGET*0.85; ckey=gridkey(seg['mid']); rcell=int(math.ceil(radius/GRID))
    years={seg['year']:0.0}
    for y,(arr,grid) in indices.items():
        if y==seg['year']: continue
        hits=[]
        for gx in range(ckey[0]-rcell,ckey[0]+rcell+1):
            for gy in range(ckey[1]-rcell,ckey[1]+rcell+1): hits.extend(grid.get((gx,gy),[]))
        best=None
...
candidates.sort(key=lambda c:(-c['n'],c['mean'],-c['seg']['len']))
selected=[]
def duplicate(c,prev):
    if adiff(c['seg']['angle'],prev['seg']['angle'])>35: return False
    try:
        return c['seg']['mid'].distance(prev['seg']['mid']) < 45000
    except Exception:
        return False
for c in candidates:
    if any(duplicate(c,p) for p in selected[-2000:]):
        continue
    selected.append(c)
...
for c in selected:
    ...
    props={'stability_id':f'bs95_{len(features)+1:06d}','kind':'геометрическая устойчивость настоящих границ АТД','method':'actual_ate_boundary_line_to_line_v95_historical_all_contours','boundary_role':'all_real_ate_boundary','boundary_scope':'internal_and_external_exposed_contours','reference_year':seg['year'],'source_name':seg['name'],'source_parent':seg['parent'],'source_type':seg['type'],'segment_length_km':round(seg['len'],3),'mid_lat':round(seg['lat'],6),'mid_lon':round(seg['lon'],6),'lat_zone':'севернее/на 59° с.ш.' if seg['lat']>=59 else 'южнее 59° с.ш.','tolerance_km':seg['tol'],'years_count':n,'years_total':len(YEARS),'stability_share':round(n/len(YEARS),4),'years':c['years'],'year_from':c['years'][0],'year_to':c['years'][-1],'time_span_years':c['years'][-1]-c['years'][0],'max_streak_slices':streak(c['years']),'mean_offset_km':round(c['mean'],3),'max_offset_km':round(c['maxd'],3),'stability_class':cls,'natural_anchor':anchor,'nearest_river':rn,'nearest_river_distance_km':None if rd is None else round(rd,3),'note':'v95: выводится реальный фрагмент границы нормальной АТЕ. Учитываются не только общие внутренние границы, но и внешние/экспонированные контуры, поэтому речные и орографические рубежи не теряются.'}
    features.append({'type':'Feature','geometry':{'type':'LineString','coordinates':cc},'properties':props})
fc={'type':'FeatureCollection','name':'boundary_stability_v95_actual_boundaries','properties':{'version':'v95','analysis':'geometric_stability_actual_ate_boundaries','years':YEARS,'years_total':len(YEARS),'source':'cleaned admin polygon exterior boundaries; internal and external contours; historical scope 1700-1923','features':len(features),'source_linelets':len(segments),'candidates':len(candidates)},'features':features}
(STAB/'boundary_stability_v95.geojson').write_text(json.dumps(fc,ensure_ascii=False,separators=(',',':')),encoding='utf-8')
summary={'version':'v95','method':'actual_ate_boundary_line_to_line_v95_historical_all_contours','years':YEARS,'years_total':len(YEARS),'source_linelets':len(segments),'candidates':len(candidates),'features':len(features),'by_count':dict(sorted(by_count.items())),'by_class':dict(by_class),'by_anchor':dict(by_anchor),'by_reference_year':dict(sorted(by_ref.items())),'note':'Historical 1700-1923 real ATE boundaries, including external/exposed contours; not centroid/graph/K-line connectors.'}
(STAB/'boundary_stability_v95_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
for name,rows in [('v95_boundary_stability_summary.csv',[('metric','value'),('version','v95'),('method',summary['method']),('years','-'.join(map(str,[YEARS[0],YEARS[-1]]))),('source_linelets',len(segments)),('features',len(features))]),('v95_boundary_stability_by_count.csv',[('years_count','segments')]+[(k,v) for k,v in sorted(by_count.items())]),('v95_boundary_stability_by_anchor.csv',[('anchor','segments')]+[(k,v) for k,v in sorted(by_anchor.items())])]:
