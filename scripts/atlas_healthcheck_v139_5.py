...
Run from the project root: python scripts/atlas_healthcheck_v139_5.py
"""
...
def check_no_early_init() -> None:
    ...
    boot_idx = max(app_text.rfind("v139_5_StabilizationLayer"), app_text.rfind("v139StabilizationLayer"))
    init_call_idx = app_text.rfind("await init();")
    if boot_idx < 0 or init_call_idx < boot_idx:
        fail("final v139.5 bootstrap does not own init()")
...
def check_known_regressions() -> None:
    ...
    known_bad = [
        "data/hydro/north_cap.geojson",
        "$('hydroToggle')",
        "$('railToggle')",
        "$('centersToggle')",
        "side.insertBefore(holder, years.parentElement)",
        "anchor.parentNode.insertBefore(holder, anchor)",
        "years.parentElement || years",
    ]
    ...
    required = [
        "openTopologyTrends",
        "v139StableTrendsBound",
        "advMinStrengthNumberV139",
        "advMaxImpedanceNumberV139",
    ...
    if "v139.2: multiyear trends modal layout guard" not in css_text:
        fail("v139.2 CSS layout guard is missing")
    for needle in ["topology-trend-years-panel-v1393","topology-trend-year-chip-v1393","topologyTrendYearsSummaryV1393","topology-trend-years-panel-v1394","topology-trend-year-era-v1394","trend-year-presets-v1394","data-years-v1394"]:
        if needle not in (app_text + "\n" + css_text):
            fail(f"v139.4 year-panel UX is missing: {needle}")

    v106_idx = app_text.find('async function v106OpenMultiyearTrendsModal()')
    if v106_idx < 0:
        fail('v106 multiyear opener missing')
    v106_body = app_text[v106_idx:app_text.find('function v106RenderMultiyearTrendChart', v106_idx)]
    for needle in ['topologyYearSelectorBitsV1395(years,state._topologyTrendYears)', 'const updateYearsPanel=()=>topologyUpdateYearsPanelV1395(body,years)', 'yearPresetButtonsHtmlV1394', 'yearsByEpochHtmlV1394']:
        if needle not in v106_body:
            fail(f'v139.5 v106 year selector wiring missing: {needle}')
    for needle in ['TOPOLOGY_CORE_YEARS_V1395', 'function topologyYearSelectorBitsV1395', 'function topologyUpdateYearsPanelV1395']:
        if needle not in app_text:
            fail(f'v139.5 shared year selector helper missing: {needle}')
