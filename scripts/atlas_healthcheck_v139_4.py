...
Run from the project root: python scripts/atlas_healthcheck_v139_4.py
"""
...
def check_no_early_init() -> None:
    ...
    boot_idx = max(app_text.rfind("v139_4_StabilizationLayer"), app_text.rfind("v139StabilizationLayer"))
    init_call_idx = app_text.rfind("await init();")
    if boot_idx < 0 or init_call_idx < boot_idx:
        fail("final v139.4 bootstrap does not own init()")
