...
Run from the project root: python scripts/atlas_healthcheck_v139_3.py
"""
...
def check_no_early_init() -> None:
    ...
    boot_idx = max(app_text.rfind("v139_3_StabilizationLayer"), app_text.rfind("v139StabilizationLayer"))
    init_call_idx = app_text.rfind("await init();")
    if boot_idx < 0 or init_call_idx < boot_idx:
        fail("final v139.3 bootstrap does not own init()")
...
def check_known_regressions() -> None:
    ...
    for needle in ["topology-trend-years-panel-v1393","topology-trend-year-chip-v1393","topologyTrendYearsSummaryV1393"]:
        if needle not in (app_text + "\n" + css_text):
            fail(f"v139.3 year-panel polish is missing: {needle}")
