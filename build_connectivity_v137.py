...
CRS_WGS='EPSG:4326'
VERSION='v137_component_connectivity'
...
method={
 ...
 'core_fix':'Ребро больше не получает один взаимоисключающий тип. Для каждой связи отдельно считаются речная, дорожная/трактовая и железнодорожная компоненты, а также болотный, рельефный, водораздельный и речной барьерный импеданс. Итоговый класс для легенды собирается из сочетания этих компонент.',
 'positive_components':{
  'rail_score':'Локальная близость железной дороги к полосе общей границы соседних АТЕ; используется с 1897 г. по year_open/year_close. Максимальный вес 3.2, радиус локального поиска 18 км.',
  'old_road_score':'Локальная близость Сибирского тракта и дорог юга Томской губернии; используются до 1897 г. Максимальный вес 2.2, радиус 25 км.',
  'navigable_river_score':'Локальная близость судоходной реки/канала. Максимальный вес 2.2 до 1876 г., 1.6 в 1876–1914 гг., 0.9 после 1914 г.; радиус 25 км. Судоходная река не снимается болотами, а проходит через них как коридор.'},
 'negative_components':{
  'wetland_impedance':'Болотность общей границы повышает импеданс. При наличии коридора штраф ослабляется, но не исчезает.',
  'relief_impedance':'Маска высот >400 м применяется только севернее 50° с.ш.; повышает импеданс, но не отменяет транспортный/речной коридор автоматически.',
  'main_watershed_impedance':'Главный водораздел между крупными MAIN_BAS повышает импеданс.',
...
(OUT/'connectivity_method_v137.json').write_text(json.dumps(method,ensure_ascii=False,indent=2),encoding='utf-8')
...
for year in years:
    ...
    for idx,row_m in edges_m.iterrows():
        ...
        rail_score = corridor_score_from_distance(rail_dist, 18.0, 3.2)
        road_score = corridor_score_from_distance(road_dist, 25.0, 2.2)
        nav_score = corridor_score_from_distance(nav_dist, 25.0, nav_max)
        ...
        # Local barriers. They now produce impedance components instead of replacing corridor classes.
        ...
        relief_impedance = 1.0 + (1.85 if high_hit else 0.0)
        watershed_impedance = 1.0 + (1.15 if main_ws_hit else 0.0)
        minor_river_impedance = 1.25 if minor_barrier else 1.0
    ...
    incident={n:{'corridor':0,'barrier':0,'blocked':0,'wetland_blocked':0,'watershed':0,'rail':0,'road':0,'river':0,'wetland_impedance':0,'relief_impedance':0,'watershed_impedance':0,'strength_sum':0.0,'impedance_sum':0.0,'impedance_count':0} for n in all_node_ids}
    for p in edge_debug:
        cls=p['adv_edge_class']
        for n in [p['adv_source_id'],p['adv_target_id']]:
            incident.setdefault(n,{'corridor':0,'barrier':0,'blocked':0,'wetland_blocked':0,'watershed':0,'rail':0,'road':0,'river':0,'wetland_impedance':0,'relief_impedance':0,'watershed_impedance':0,'strength_sum':0.0,'impedance_sum':0.0,'impedance_count':0})
