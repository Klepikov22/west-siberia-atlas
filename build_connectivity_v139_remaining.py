...
# v139: include all available Irtysh / Иртыш lines as navigable corridor evidence.
try:
    rivers_for_irtysh=read(DATA/'hydro/rivers_full.geojson')
    mask=rivers_for_irtysh.apply(lambda r: ('irtysh' in str(dict(r)).lower()) or ('иртыш' in str(dict(r)).lower()), axis=1)
    irtysh_extra=rivers_for_irtysh[mask].copy()
    if len(irtysh_extra):
        irtysh_extra['v139_added_as_navigable']='Irtysh full course from rivers_full'
        nav=pd.concat([nav, irtysh_extra[nav.columns.intersection(irtysh_extra.columns).tolist() + [c for c in irtysh_extra.columns if c not in nav.columns]]], ignore_index=True, sort=False)
except Exception as e:
    print('v139 Irtysh enrichment failed', e)
...
method={
 ...
 'base_graph':'Исходной рамкой остаётся обычный граф геометрической смежности topology_YYYY.geojson. v139 не заменяет его, а присваивает каждому ребру независимые компоненты коридоров и барьеров.',
 'core_fix':'Ребро не получает один взаимоисключающий тип. Для каждой связи отдельно считаются речная, дорожная/трактовая и железнодорожная компоненты, а также болотный, рельефный, водораздельный и речной барьерный импеданс. v139 учитывает локальную длину коридора или барьера около общей границы: короткое касание даёт меньший вклад, протяжённый локальный коридор или барьер — больший.',
 'positive_components':{
 ...
 'negative_components':{
  ...
  'relief_impedance':'Маска высот >400 м применяется западнее 84° в.д. севернее 50° с.ш., восточнее 84° в.д. севернее 49° с.ш.; повышает импеданс пропорционально локальной длине пересечения общей границы с орографическим барьером, но не отменяет транспортный/речной коридор автоматически.',
...
(OUT/'connectivity_method_v139.json').write_text(json.dumps(method,ensure_ascii=False,indent=2),encoding='utf-8')
...
for year in years:
    ...
    for idx,row_m in edges_m.iterrows():
        ...
        relief_threshold = 49.0 if (c_lon is not None and c_lon >= 84.0) else 50.0
        relief_allowed = bool(c_lat is not None and c_lat >= relief_threshold)
        relief_local_km = local_polygon_contact_km(high_idx, contact) if relief_allowed else 0.0
        high_hit = bool(relief_allowed and (relief_local_km > 0 or high_idx.dwithin(contact,3000)))
        ...
        props.update({
          ...
          'adv_contact_local_test':'cost-graph shared-border band / nearest-boundary proxy; corridors and barriers are counted as local cost modifiers, not as polygon-wide presence flags',
